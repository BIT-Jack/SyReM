...
class JointInteractionDataset(Dataset):
    def __init__(self, scenarionames, stage, para, mode, filters=True):
        self.stage= stage
        self.para = para
        ...
        if stage == 'train':
            ...
            for i in range(len(scenarionames)):
                data = np.load(abs_dir+stage+'/'+stage+'_'+scenarionames[i]+'.npz', allow_pickle=True)
                ...
                data_mask = np.load(abs_dir+stage+'/mask_train_'+scenarionames[i]+'.npz', allow_pickle=True)
                self.mask.append(data_mask['mask'])
            ...
            for val_name in scenarionames:
                data = np.load(abs_dir+stage+'/'+stage+'_'+val_name+'.npz', allow_pickle=True)
                self.T.append(data['trajectory'])
                self.M.append(data['maps'])
                self.L.append(data['lanefeature'])
                self.N_agents.append(data['nbagents'])
                ...
                data_mask = np.load(abs_dir+stage+'/mask_val_'+val_name+'.npz', allow_pickle=True)
                self.mask.append(data_mask['mask'])
