...
from utils.der_gssp_buffer_new import Buffer as Buffer
    ...
    def observe(self, inputs, labels, task_id=None):
        ...
        for _ in range(self.alj_nepochs):
            ...
            if not self.buffer.is_empty():
                ...
                #------------To enhance the plasticity----------------------------
                buf_inputs2, buf_labels2, sampled_task_id2 = self.buffer.get_data(
                    self.args.minibatch_size, transform=self.transform, pl=True)
            ...
            #-------------enhancement------------------
            if not self.buffer.is_empty():
                outputs2 = self.net(buf_inputs2)           
                loss += self.loss(outputs2, buf_labels2)
