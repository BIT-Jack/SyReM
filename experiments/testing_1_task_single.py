...
def test_1_task_single(task_index, args):

    cl_method_name = args.model
    scenario_info = args.scenario_info
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    result_log = open(result_dir+'/'+'baseline_task_'+str(task_index)+'.txt','w')
    ...
    model.encoder.load_state_dict(torch.load(baseline_dir+'/'+'1_epoch_'+args.model+'_'+'tasks_'+str(task_index)+'_encoder'+'.pt', map_location='cuda:0'))
    model.decoder.load_state_dict(torch.load(baseline_dir+'/'+'1_epoch_'+args.model+'_'+'tasks_'+str(task_index)+'_decoder'+'.pt', map_location='cuda:0'))
    ...
    baseline_result['fde'] = np.mean(FDE)
    baseline_result['mr'] = np.mean(MR)*100
